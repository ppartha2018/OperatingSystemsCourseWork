...
//allocate pid for new threads
static pid_t get_new_pid(struct thread t){
  int i;
  for (i=2; i<102; i++){
...
thread_create(const char *name)
{
	...
	//thread->pid = get_new_pid(*thread);
	thread->pid = pid_alloc();
	DEBUG(DB_SYSCALL, "In thread pid allocated = %d\n",thread->pid);
...
thread_bootstrap(void)
{
	...
	//process_table = (struct thread*)kmalloc((100)*sizeof(struct thread));
	pid_bootstrap();

	//create the file lock used for file i/o
	file_lock = lock_create("file_lock");

	/* create the execv lock to be used to ensure atomic operation while executing new program */
