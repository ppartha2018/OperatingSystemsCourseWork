...
q_remtail(struct queue *q)
{
    ...
    assert(!q_empty(q));
    ret = q->data[(q->nextwrite-1)%q->size];
    q->nextwrite = (q->nextwrite-1)%q->size;
    return ret;
}

void *
q_peektail(struct queue *q){
    void *ret;
    ...
    assert(!q_empty(q));
    ret = q->data[(q->nextwrite-1)%q->size];
//    kprintf("Value of head = %d tail %d\n",q->nextread, q->nextwrite);
    return ret;
}
